...
describe('topologicalSort', () => {
	...
	it('Should compute topological sort tricky case.', () => {
		...
		expect(sorted.size()).toEqual(4);
		expect(sorted.includes('b')).toBe(true);
		expect(sorted.includes('c')).toBe(true);
		expect(sorted.includes('d')).toBe(true);

		expect(sorted[sorted.size() - 1]).toEqual('e');
	...
	it('Should exclude source nodes with multiple cycles.', () => {
		...
		expect(sorted.includes('b')).toBe(false);
	});
