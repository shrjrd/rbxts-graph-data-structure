const JSON_stringify = (value: unknown) => game.GetService('HttpService').JSONEncode(value);
const JSON_parse = (value: string) => game.GetService('HttpService').JSONDecode(value);
...
describe('serializeGraph', () => {
	...
	it('should not duplicate nodes when they are objects', () => {
		...
		const stringData = JSON_stringify({
			nodes: [nodeA, nodeB],
			links: [{ source: nodeA, target: nodeB, props: { type: 'foo' } }],
		});

		const serializedGraph = JSON_parse(stringData) as Serialized<{ id: number; title: string }, { type: string }>;
	...
	it.skip('should return a graph with type inferred from the input', () => {
		...
		//expectTypeOf(graph).toEqualTypeOf<Graph<{ id: number; title: string }, { type: string }>>();
	});
