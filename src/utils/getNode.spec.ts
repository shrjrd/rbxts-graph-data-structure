...
describe('getNode', () => {
	...
	it('should throw when the node is not found', () => {
		const graph = new Graph<Node>();
		expect(() => getNode(graph, (n) => n.id === 'nope')).toThrowError();
	});

	it('should throw when more than one node is found', () => {
		const graph = new Graph<Node>();
		const node1 = { id: '1', type: 'foo' };
		const node2 = { id: '2', type: 'foo' };
	...
	it.skip('should not trigger a type error when the link props are set', () => {
